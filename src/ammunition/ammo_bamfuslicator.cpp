...
// Operator for the zombie_type enum
zombie_type& operator++(zombie_type& zombie, int32_t)
{
	switch (zombie)
	{
	case zombie_type::zombie_type_normal: return zombie = zombie_type::zombie_type_fast;
	case zombie_type::zombie_type_fast: return zombie = zombie_type::zombie_type_ogre;
	case zombie_type::zombie_type_ogre: return zombie = zombie_type::zombie_type_normal;
	default: return zombie = zombie_type::zombie_type_normal;
	}
};
...
void Ammo_Bamfuslicator(edict_t* self, vec3_t start, vec3_t aimdir, zombie_type zombie_type)
{
	...
	switch (zombie_type)
	{
	case zombie_type::zombie_type_normal:
		SP_monster_zombie(monster);
		break;
	case zombie_type::zombie_type_fast:
		SP_monster_zombie_fast(monster);
		break;
	case zombie_type::zombie_type_ogre:
	...
	edict_t** within_bounds_list = &within_player_bounds[0];

	// now see if we are trying to spawn the zombie within another entity...
	int32_t num_within_player_bounds = gi.BoxEdicts(vec_absmin, vec_absmax, within_bounds_list, 64, AREA_SOLID);

	for (int32_t edict = 0; edict < num_within_player_bounds; edict++)
	{
		if (!strncmp(within_bounds_list[edict]->classname, "monster_", 8)) // check for any monster
		{
			gi.sound(self, CHAN_VOICE, gi.soundindex("weapons/bamfuslicator/spawn_failed.wav"), 1, ATTN_NORM, 0);
			//todo: push out
			Edict_Free(monster);
	...
	within_bounds_list = &within_monster_bounds[0];

	// now see if we are trying to spawn the zombie within another entity...
	int32_t num_within_monster_bounds = gi.BoxEdicts(vec_absmin, vec_absmax, within_bounds_list, 64, AREA_SOLID);

	for (int32_t edict = 0; edict < num_within_monster_bounds; edict++)
	{
		if (!strncmp(within_bounds_list[edict]->classname, "worldspawn", 11))
		{
			gi.sound(self, CHAN_VOICE, gi.soundindex("weapons/bamfuslicator/spawn_failed.wav"), 1, ATTN_NORM, 0);
			Edict_Free(monster);
			return;
