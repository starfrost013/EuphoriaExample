...
#define GAME_VERSION GAME_NAME " v0.1.0"
#define GAME_VERSION_FULL GAME_VERSION " " __DATE__ __TIME__
...
zombie_type& operator++(zombie_type& zombie, int32_t);
...
typedef struct gitem_s
{
	...
	zombie_type	spawn_type;		// Type of entity to spawn (item-specific)
...
void Item_SetName();
gitem_t* Item_FindByPickupName(const char* pickup_name);
gitem_t* Item_FindByClassName(const char* classname);
...
edict_t* Game_FindEdictByValue(edict_t* from, int32_t fieldofs, const char* match);
edict_t* Game_FindEdictsWithinRadius(edict_t* from, vec3_t org, float rad);
edict_t* Edict_PickTarget(const char* targetname);
...
void	GameUI_Send(edict_t* ent, const char* ui_name, bool enabled, bool activated, bool reliable);
void	GameUI_SetText(edict_t* ent, const char* ui_name, const char* control_name, const char* text, bool reliable);
void	GameUI_SetImage(edict_t* ent, const char* ui_name, const char* control_name, const char* image_path, bool reliable);
...
// g_misc.c
//
void ThrowHead(edict_t* self, const char* gibname, int32_t damage, int32_t type);
void ThrowClientHead(edict_t* self, int32_t damage);
void ThrowGib(edict_t* self, const char* gibname, int32_t damage, int32_t type);
...
//
void Server_Command();
bool Server_IsClientBanned(const char* from);
...
// game_client_spawn.c 
// 
edict_t* Player_SpawnSelectFarthest(const char* spawn_class_name);
edict_t* Player_SpawnSelectRandom(const char* spawn_class_name);
